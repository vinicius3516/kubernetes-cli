...
parser = argparse.ArgumentParser(
    description="Filter pods and their respective images from all namespaces on kubernetes cluster",
    prog="kube-get-images.py"
)
parser.add_argument("-n", "--namespaces", help="Filter pods and images from specific namespaces")


TABLE_HEADERS = [
    f"{Fore.LIGHTWHITE_EX}{Style.BRIGHT}Pods{Style.RESET_ALL}", 
    f"{Fore.LIGHTWHITE_EX}{Style.BRIGHT}Images{Style.RESET_ALL}"
]
...
def str_to_cyan(string: str) -> str:
    return f"{Fore.CYAN}{string}{Style.RESET_ALL}"

def str_to_green(string: str) -> str:
...
def main():
    ...
    if args.namespaces is None:
        ...
        pods = get_pods_from_namespace(args.namespaces)
